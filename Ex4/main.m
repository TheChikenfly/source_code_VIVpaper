...
global ILVIVBool; global Vrelvect;
% add onsas block
...
velocitiesbis = [0.01:0.01:0.28 0.31:0.03:1.4];
[tf,idx] = ismember(round(velocitiesbis,2),round(velocities,2));
idx(idx==0) = [];
for indvel = 34:NR%[1:27 29:NR]%idx(23:end)%18:NR % 55
    tic;
    Vrelvect = zeros(6*(numElements+1), finalTime/dt);
    vwindMax = velocitiesbis(indvel)
    ...
    save(sprintf('Ysolutions\\YCoralN=0Vr_va=%.3f_Nelem=%d_FT%d_dt=%.4f.mat', vwindMax, numElements, finalTime,dt), 'ydefNumall', 'xdefNumall', 'zdefNumall', 'Vrelvect')
    elapTime2 = toc ;
